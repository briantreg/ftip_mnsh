...
afl_NEW_home = pd.DataFrame({'team':teams, 
          'opponent':opponents, 
          'home_game': 1, 
          'Result': 1, 
          'fw_game_id': games, 
          ...
          'year': year})
afl_NEW_away = pd.DataFrame({'team':opponents, 
          'opponent':teams, 
          'home_game': 0, 
          'Result': 1, 
          'fw_game_id': games, 
          ...
          'round_char': str(round_n),
          'year': year})
afl_NEW = pd.concat([afl_NEW_home, afl_NEW_away], sort = False)
...
afl_NEW = pd.concat([afl_DF[afl_DF['team'] ==  'none'], afl_NEW], sort = False)
afl_NEW = afl_NEW.fillna(0.0)
afl_DF = pd.concat([afl_DF, afl_NEW], sort=False)
...
round_index = afl_DF[0:][['year','season_round']] #New dataset with year and season_round
...
afl_NEW = afl_DF[afl_DF['fw_game_id'].isin(range(1,10))]
afl_DF = afl_DF[~afl_DF['fw_game_id'].isin(range(1,10))]
#%%
