...
def scaleMLData(train_data, X_cols):
...
def selBoruta(X_test, X_test_scaled, X_train, X_train_scaled, Y_trainval):
    ...
    feat_selector = BorutaPy(rf, n_estimators=n_estimators, verbose=verbose, random_state=random_state, max_iter = 100)
...
def scoreProbs(classifier, X_test_data, Y_test_data):
...
def scoreMonash(classifier, X_test_data, Y_test_data):
    Y_test_probabilities = scoreProbs(classifier, X_test_data, Y_test_data)
    Y_test_probabilities['monash_score'] = ((1 + np.log2(Y_test_probabilities.iloc[:,1])) * Y_test_probabilities['model_target']) + (1 + np.log2(1 - Y_test_probabilities.iloc[:,1])) * (1 - Y_test_probabilities['model_target'])
    return Y_test_probabilities
...
def dataScored(new_data, X_cols, X_scaler, feat_selector, clf):
    ...
    X_scored = X_scored.join(new_data[['fw_game_id','home_game','team','opponent']])
    X_scored = utl.combineHAScores(X_scored)
    return X_scored 
...
def avgModel(*models):
    ...
    for m in models:
        if first == 1:
            ...
            mergemod = pd.merge(mergemod,
                     m[cols],
                     on = 'team',
                     how = 'left')
    mergemod['Win'] = mergemod.iloc[0:,1:len(mergemod.columns)].sum(axis = 1) / (len(mergemod.columns) - 1)
    mergemod = mergemod[['team', 'Win']]
    mergemod['backOdds'] = 1 / mergemod['Win']
    mergemod['layOdds'] = 1 / (1 - mergemod['Win'])
    return mergemod
