...
def scrapeGameData(soup, game_id, afl_url):
...
def prettyPlayerStatTable(table):
    table = pd.read_html(str(table))
    table = table[0]
    colunmsPlayerStats = list(table.iloc[0,:])
    table = table.iloc[1:,:]
    table.columns = colunmsPlayerStats 
    return table
       
def scrapeRound(soup): #Code stolen from scrapeGameData() in main scraper
    #isolate table with the initial info and get date and year
    info_table = infoTable(soup)
    date = dateTime(info_table)
    year = date.year
    ...
    if 'Final' in the_round:
        ...
        round_char = re.sub('[^0-9]', '', the_round)
        season_round = int(re.sub('[^0-9]', '', the_round))
    return year, round_char, season_round

def appendGameDetails(tablePlayer, tablePlayerLink, year, round_char, season_round, fw_game_id):
    table = pd.DataFrame(tablePlayer.Player)
    table['year'] = year
    table['round_char'] = round_char
    table['season_round'] = season_round
    table['fw_game_id'] = fw_game_id
    table = pd.merge(table,
                     tablePlayerLink,
                     on = 'Player')
    return table

def cleanPrices(column):
