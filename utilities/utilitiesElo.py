...
def eloProbWin(elo_t, elo_o, flagHome, HGA, m, p, returnMargin):
    if flagHome == 0:
        HGA = - HGA
    nume2 = - (elo_t - elo_o + HGA)
    sum1 = nume2 / m 
    p1 = 1 / (math.pow(10, sum1) + 1) + 0.000001
    
    if returnMargin == True:
        marginNumerator = np.log((1 - p1)/ p1 )
        p1Margin = -marginNumerator / p
        return p1, p1Margin 
    else:
        return p1

def eloNew(eloWin, eloLose, K, flagHome, HGA, m, p, KMargin):
    p1, p1Margin = eloProbWin(eloWin, eloLose, flagHome, HGA, m, p, returnMargin = True)
    p0 = 1 - p1
    eloWinNew = eloWin + ( K * (1 - p1) ) +  ( KMargin * (1 - p1Margin) ) #Change for winner
    eloLoseNew = eloLose + ( K * (0 - p0) ) +  ( KMargin * (1 - p1Margin) ) #Change for loser
    return eloWinNew, eloLoseNew

def eloCreate(data, colTeam, colOpponent, colHomeGame, colWin, colMargin, eloInitialScore, HGA, K, m, colEloOut, colSeason, regressParam, p, KMargin):
    ...
    for i in range(0, n):
        ...
        p1 = eloProbWin(eloTeamCurrent, eloOpponentCurrent, teamHome, HGA, m, p, returnMargin = False) #Probability team will win
        
        if(x[[colWin]].values == 1): #New elos if team won
            eloTeamNew, eloOpponentNew = eloNew(eloTeamCurrent, eloOpponentCurrent, K, teamHome, HGA, m, p, KMargin)
        if(x[[colWin]].values == 0): #New elos if team won
            margin = -margin
            eloOpponentNew, eloTeamNew = eloNew(eloOpponentCurrent, eloTeamCurrent, K, homeOpponent, HGA, m, p, KMargin)
...
def eloGridSearch(data, kFold, eloParams, scoreFunc, excludeFirstN, colTeam = 'team', colOpponent = 'opponent', colHomeGame = 'home_game', colWin = 'model_target', colMargin = 'Margin', colEloOut = 'eloProb', colSeason = 'year',     test_ids_file = 'test_fw_ids.pkl'):
    ...
    kfoldSplits = kfoldSplit(dataXval, kFold)
    ...
    for row in kfoldGrid.itertuples(index = False):
        ...
        dataScore = eloCreate(data, colTeam, colOpponent, colHomeGame, colWin, colMargin, eloInitialScore, HGA, K, m, colEloOut, colSeason, regressParam, p, KMargin)
